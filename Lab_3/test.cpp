...
struct system_for_solving//ok
{
	possible_permutation equation_first;
	possible_permutation equation_second;
};
...
type search_number_representation_of_bigram(char first, char second) //ok
{
	...
	return mod(temp_first * lower_alphabet.length() + temp_second,pow(lower_alphabet.length(),2));
}
...
possible_permutation* permutation_all_bigrams_with_each_other_theoretical_and_practice(possible_permutation* combination_bigram, string* theoretical_statistic, string* practical_statistic, type size_of_array_frequent_bigrams)//ok
{
	...
	for (type i = 0; i < size_of_array_frequent_bigrams; i++)
	{
		...
		{
			combination_bigram[i].first_.bigram = theoretical_statistic[i];
			combination_bigram[i].first_.value = search_number_representation_of_bigram(combination_bigram[i].first_.bigram[0], combination_bigram[i].first_.bigram[1]);

			combination_bigram[i].second_.bigram = practical_statistic[j];
			combination_bigram[i].second_.value = search_number_representation_of_bigram(combination_bigram[i].second_.bigram[0], combination_bigram[i].second_.bigram[1]);

			counter++;

			cout << endl << "\t" << counter << "\t" << combination_bigram[i].first_.bigram << "\t" << combination_bigram[i].first_.value << "\t" << combination_bigram[i].second_.bigram << "\t" << combination_bigram[i].second_.value << endl;
		}
	}
...
vector<key> build_all_possible_key(possible_permutation* combination_bigram, vector<key> keys)
{
	...
	for (type i = 0; i < 25; i++)
	{
		for (type j = 0; j < 25; j++)
		{
			keys=solving_system(keys, (combination_bigram[i]).first_.value, (combination_bigram[j]).first_.value, (combination_bigram[i]).second_.value, (combination_bigram[j]).second_.value,  pow(lower_alphabet.length(), 2));
		}
	}
...
key choose_text_decryption(string cipher_text, vector<key> keys)//ok
{
	...
	for (type i = 0; i < keys.size(); i++)
	{
		if (keys[i].system_solution == true)
		{
			...
			if (check_the_content_of_the_text(text) && text.length() == cipher_text.length())
			{
...
key crack_key(bigram_tools* theoretical_bigram_sorted, string cipher_text)
{
	...
	combination_bigram=permutation_all_bigrams_with_each_other_theoretical_and_practice(combination_bigram, theoretical_statistic, practical_statistic, 5);


	keys=build_all_possible_key(combination_bigram, keys);
...
int main()
{
	...
	else
	{
		...
		{   // block with text encypted process 

			key key_encryption;  key_encryption.first = 13;   key_encryption.second = 151;
			...
			string temp_ = affine_encryption(encryption_text, key_encryption);
			cout << endl << "Encryption text: " << temp_ << endl;

			cout << endl << "Key: (" << key_encryption.first << "," << key_encryption.second << ")" << endl;

			// block with text decryption  process

			key key_decryption;

			key_decryption.first = 0;
			key_decryption.second = 0;
			...
			//fout_for_decryption_text << temp_;

			decryption_text = temp_;

			cout << endl << "Decryption text: " << decryption_text << endl;
			...
			/*cout << endl << "Text: " << _temp << endl;

			cout << endl << "Key: (" << key_decryption.first << "," << key_decryption.second << ")" << endl;*/
		...
		/*{   // block with text decryption  process
			...
			decryption_text = we_make_the_text_of_even_length(get_all_text_in_one_string(fin_for_decryption_text, encryption_text, one_string_decryption_text));
			cout << endl << "Decryption text: " << decryption_text << endl;

			cout << endl << "Process sorting statistics bigram: " << endl;
			sorting_matrix_in_descending(theoretical_bigram_sorted, decryption_text);

			string _temp = affine_decryption(decryption_text, crack_key(theoretical_bigram_sorted, decryption_text));
			cout << endl << "Text: " << _temp << endl;
